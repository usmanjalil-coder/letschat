...
<div class="container chat-container">
    <div class="conversation-list px-2">
        ...
        @isset($all_users)
            @foreach ($all_users as $key => $users)
                <div class="conversation-item" data-id="{{ $users['id'] }}">
                    <div>
                        <div class="all-online-user" id="online-user-{{ $users['id'] }}"></div>
                        <img src="{{ isset($users['image']) ? asset('storage').'/'. $users['image'] : asset('images/person.jpg')}}" class="rounded-circle" height="30px" width="30px" alt="">
                        <strong>{{ ucfirst($users['name']) }}- &nbsp;&nbsp;&nbsp;<span class="user-status"></span></strong>
                        <span id="typing-{{ $users['id'] }}" class="timestamp d-none">Typing...</span>
                    </div>
                    {{-- <div>
    ...
    <div class="chat-area position-relative" >
        ...
        {{-- for audio  --}}
        <div id="audio-recording-ui" class="audio-recording-ui d-none position-absolute" style="bottom: 70px; left: 26px;">
